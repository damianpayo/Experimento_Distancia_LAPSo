...
```
## Logarithmic and labeled values
...
We eliminated 938 observations from the total of 4968 with this method.
...
data_slopes <- data_slopes %>%
  summarise(nsub, condicion, slope, intercept, Frec) %>%
  unique()
...
```
### Outliers by Slopes and Incomplete filtering
...
### Reescritura de data_clean con los datos que teniamos, sumando los promedios y desviación standar.

```{r}
data_prom <- data_clean %>%
  group_by(condicion, dist_log, Frec) %>%
  summarise(condicion,
            distancia,
            respuesta,
...
```
```{r echo=FALSE}
ggplot(data_clean %>% filter(Frec == '1k'), aes(x=dist_log, y=resp_log, color=condicion)) +
...
The results show a significant difference between Low-Wide and Low-High for 1kHz, justifying the data shown in the previous graph.
...
In the ANOVA analysis, we show that the Condition factor is significant for determining slope variation in experiments performed with a cutoff frequency at 1kHz (p-value < 0.001), but not for studies performed with a cutoff frequency of 4kHz.
...
data_clean <- data_clean %>%
  ...
  summarise(resp_log, 
            ...
            bloque,
            prom_lin = 
            promedio = mean(resp_log), 
            sd = sd(resp_log), 
            std = std.error(resp_log))


G_Prom_All <- ggplot(data_clean %>% filter(Frec == '1k'), aes(x=dist_log,
                               y=resp_log,
                               color = condicion)) +
  #geom_point() +
  ...
  # scale_x_log10() +
  # scale_y_log10() +
  geom_errorbar(aes(ymin=min(resp_log), ymax=max(resp_log)), width = 0.1) +
